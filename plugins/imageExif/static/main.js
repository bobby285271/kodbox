...
kodReady.push(function(){
	var menuOpt = {
		'image-exif':{
			...
			callback:function(action,option){
				...
				var request = '{{pluginApi}}getExif&path='+param.path;
	...
	Events.bind('rightMenu.show.menu-file',function($menuAt,$theMenu){
		if($('.context-menu-active').hasClass('menu-tree-file') ){
			var param = ui.tree.makeParam();
		}else{
			var param = ui.path.makeParam();
		}
		var ext = pathTools.pathExt(param.path);
		var hideClass = 'hidden';//'disabled' hideClass
		if (_.includes(['jpg','jpeg','png','bmp'],ext)){
