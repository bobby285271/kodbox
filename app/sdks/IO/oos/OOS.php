...
class OOS {
	...
	public static function setSigningKey($keyPairId, $signingKey, $isFile = true) {
		...
		self::__triggerError('OOS::setSigningKey(): Unable to open load private key: ' . $signingKey, __FILE__, __LINE__);
	...
	private static function __triggerError($message, $file, $line, $code = 0) {
		if (self::$useExceptions) {
			throw new OOSException($message, $file, $line, $code);
		} else {
	...
	private function __execReponse($rest, $function, $noBody = 0, $params = array(), $code = 200) {
		...
		if ($rest->error !== false) {
			...
			self::__triggerError(sprintf('OOS::' . $function . '('.$param.'): [%s] %s', $rest->error['code'], $rest->error['message']), __FILE__, __LINE__);
	...
	public static function listBuckets($detailed = false) {
		$rest = new OOSRequest('GET', '', '', self::$endpoint);
		$rest = $rest->getResponse();
	...
	public static function getBucket($bucket, $prefix = null, $marker = null, $maxKeys = null, $delimiter = null, $returnCommonPrefixes = false) {
		...
		$nextMarker = null;
		do{
			$rest = new OOSRequest('GET', $bucket, '', self::$endpoint);
			if ($maxKeys == 0) {
				$maxKeys = null;
			}
	...
	public static function putBucket($bucket, $acl = self::ACL_PRIVATE, $location = false) {
		$rest = new OOSRequest('PUT', $bucket, '', self::$endpoint);
	...
	public static function deleteBucket($bucket) {
		$rest = new OOSRequest('DELETE', $bucket, '', self::$endpoint);
		$rest = $rest->getResponse();
	...
	public static function inputFile($file, $md5sum = true) {
		if (!file_exists($file) || !is_file($file) || !is_readable($file)) {
			self::__triggerError('OOS::inputFile(): Unable to open input file: ' . $file, __FILE__, __LINE__);
	...
	public static function inputResource(&$resource, $bufferSize = false, $md5sum = '') {
		if (!is_resource($resource) || (int) $bufferSize < 0) {
			self::__triggerError('OOS::inputResource(): Invalid resource or buffer size', __FILE__, __LINE__);
		...
		if ($bufferSize === false) {
			if (fseek($resource, 0, SEEK_END) < 0 || ($bufferSize = ftell($resource)) === false) {
				self::__triggerError('OOS::inputResource(): Unable to obtain resource size', __FILE__, __LINE__);
	...
	public static function putObject($input, $bucket, $uri, $acl = self::ACL_PRIVATE, $metaHeaders = array(), $requestHeaders = array(), $storageClass = self::STORAGE_CLASS_STANDARD, $serverSideEncryption = self::SSE_NONE) {
		...
		$rest = new OOSRequest('PUT', $bucket, $uri, self::$endpoint);
	...
	public static function getUploadId($bucket, $uri, $metaHeaders = array(), $requestHeaders = array()) {
		$rest = new OOSRequest('POST', $bucket, $uri, self::$endpoint);
	...
	public function listParts($bucket, $uri, $uploadId) {
		$rest = new OOSRequest('GET', $bucket, $uri, self::$endpoint);
		$rest->setParameter('uploadId', $uploadId);
	...
	public static function getObject($bucket, $uri, $requestHeaders = array(), $saveTo = false) {
		...
		if ($rest->response->error !== false) {
			self::__triggerError(sprintf("OOS::getObject({$bucket}, {$uri}): [%s] %s", $rest->response->error['code'], $rest->response->error['message']), __FILE__, __LINE__);
	...
	public static function getObjectInfo($bucket, $uri, $returnInfo = true) {
		$rest = new OOSRequest('HEAD', $bucket, $uri, self::$endpoint);
		$rest = $rest->getResponse();
		...
		if ($rest->error !== false) {
			self::__triggerError(sprintf("OOS::getObjectInfo({$bucket}, {$uri}): [%s] %s", $rest->error['code'], $rest->error['message']), __FILE__, __LINE__);
	...
	public static function deleteObject($bucket, $uri) {
		$rest = new OOSRequest('DELETE', $bucket, $uri, self::$endpoint);
		$rest = $rest->getResponse();
	...
	public function deleteObjects($bucket, $data) {
		...
		$rest = new OOSRequest('POST', $bucket, '', self::$endpoint);
		$rest->setHeader('Content-Type', 'application/octet-stream');
		$rest->setHeader('Content-MD5', self::__base64(md5($body)));
		$rest->setHeader('Content-Length', strlen($body));
	 ...
	 * https://oos-cn.ctyunapi.cn/docs/oos/OOS%E5%BC%80%E5%8F%91%E8%80%85%E6%96%87%E6%A1%A3-v6.pdf
	 * @param string $bucket     Bucket name
	 * @param string $uri        Object URI
	 ...
	 *
	 * @internal Used by OOSRequest::getResponse()
	 *
...
final class OOSRequest {
	...
	public function getResponse() {
		...
		if (OOS::$proxy != null && isset(OOS::$proxy['host'])) {
			curl_setopt($curl, CURLOPT_PROXY, OOS::$proxy['host']);
			curl_setopt($curl, CURLOPT_PROXYTYPE, OOS::$proxy['type']);
			if (isset(OOS::$proxy['user'], OOS::$proxy['pass']) && OOS::$proxy['user'] != null && OOS::$proxy['pass'] != null) {
				curl_setopt($curl, CURLOPT_PROXYUSERPWD, sprintf('%s:%s', OOS::$proxy['user'], OOS::$proxy['pass']));
			}
		}
		...
		if (OOS::hasAuth()) {
			...
			if ($this->headers['Host'] == 'cloudfront.amazonaws.com') {
				$headers[] = 'Authorization: ' . OOS::__getSignature($this->headers['Date']);
			} else {
				if (OOS::$signVer == 'v2') {
					$headers[] = 'Authorization: ' . OOS::__getSignature(
							$this->verb . "\n" .
					...
					$amzHeaders = OOS::__getSignatureV4(
		...
		if (OOS::$progressFunction) {
			curl_setopt($curl, CURLOPT_NOPROGRESS, false);
			curl_setopt($curl, CURLOPT_PROGRESSFUNCTION, OOS::$progressFunction);
		}
